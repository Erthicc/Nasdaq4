# worker.py
"""
Resilient worker with robust column normalization.
- Handles tuple/multiindex column names from various data sources.
- Uses yfinance + Stooq fallback as before.
- Always writes raw-results-{JOB_INDEX}.json artifact.
"""
import os
import sys
import json
...
# --- helpers and robust column normalization ---
def normalize_and_canonicalize_columns(df):
    """
    Convert df.columns (which might be tuples or weird names) into canonical:
    'Open','High','Low','Close','Adj Close','Volume' when possible; otherwise string.
    This prevents errors like "'tuple' object has no attribute 'capitalize'".
    """
    cols_out = []
    for c in df.columns:
        # Turn tuple -> string
        ...
        # heuristics to map to canonical column names
        if 'open' in low and 'adj' not in low:
            cols_out.append('Open')
        elif 'high' in low:
            cols_out.append('High')
        elif 'low' in low:
            cols_out.append('Low')
        elif 'close' in low and 'adj' in low:
            cols_out.append('Adj Close')
        elif 'close' in low:
            cols_out.append('Close')
        elif 'volume' in low:
            cols_out.append('Volume')
        else:
            # fallback: if column looks like a date index name, keep it
            cols_out.append(s)
    df.columns = cols_out
    return df

# --- download nasdaqlisted (kept robust) ---
...
def fetch_stooq_csv(ticker, days=440):
    ...
    try:
        ...
        if df.empty:
            return None
        df = df.rename(columns=lambda c: c.capitalize() if isinstance(c, str) else str(c))
        df = df.set_index("Date").sort_index()
        # normalize columns
        df = normalize_and_canonicalize_columns(df)
...
# --- yfinance wrapper with normalization & fallback ---
    ...
    for attempt in range(1, retries+1):
        try:
            ...
            if not {'Open','High','Low','Close','Volume'}.issubset(set(df.columns)):
                raise ValueError(f"missing columns from yfinance: {list(df.columns)}")
...
# --- indicators (unchanged but robust) ---
